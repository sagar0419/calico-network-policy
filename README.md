...
It will give you an output like this. In the below screenshot, you can see that we have created the storage class with the name longhorn.
Screenshot of storage class on the target Kubernetes cluster.2. By adding configmap on the target cluster.
...
Screenshot of configmap deployed in the velero namespace.Backup:
...
Screenshot of the velero backup command. wordpress-backup is the name of our  backup. You can also check the backup on the S3 bucket which you have created.
To restore the backup on the target Kubernetes cluster run the below-mentioned command on the target cluster.
velero restore create --from-backup <backup-name>--include-namespaces demo
...
Screenshot of restored application on the target Kubernetes cluster.As you can see in the above screenshot PVC is created with the storage class "rook-cephfs". 
Below is the screenshot of the storage class used in the target class.
Screenshot of storage class used in the target Kubernetes cluster.Let's check the WordPress site on the browser.
Screenshot of the application running on the target Kubernetes Cluster.As you can see in the above screenshot our application is migrated along with the data in the PVC.
Automating Backups
